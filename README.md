...
*This is a fork of the original [Bullish Webstore](https://github.com/supertestnet/superstore) project.*
...
Just visit this link: https://thebullishbitcoiner.github.io/bullishwebstore/
...
- [x] Portable shipping status -- previously when you click Shipped on an order, Bullish Webstore stored the fact that you shipped that product in your browser's local storage. If you did that on your computer and then went to manage your store on your phone, your phone wouldn't know you shipped the product, so it would still show it as needing to be shipped. (Unless the money went into a bitcoin address and then you withdrew it.) Now I made it so that when you click Shipped on an order, Bullish Webstore sends a dm that references the shipped order in a tag, and then if you switch devices Bullish Webstore can load those dms and thus know which orders you shipped.
